...
function isSameCircuit(id1, id2) {
  ...
  if (
    (id1 === "austin" && id2 === "americas") ||
    (id1 === "americas" && id2 === "austin")
  )
    return true;
  return false;
}
...
const minimapSvg = d3
  .select("#minimap-container")
  .append("svg")
  .attr("width", "100%")
  .attr("height", "100%")
  .attr("viewBox", `0 0 ${minimapWidth} ${minimapHeight}`);
const minimapProjection = d3
  .geoEquirectangular()
  .scale(38)
  .translate([minimapWidth / 2, minimapHeight / 2]);
...
function drawMap() {
  ...
  atmosphere = svg
    ...
    .attr("cy", height / 2)
    .attr("r", initialScale)
    .style("fill", "url(#atmosphereGradient)")
    .attr("pointer-events", "none");

  let circuits = f1Json.MRData.CircuitTable.Circuits;
  const trackPromises = circuits.map((c) =>
    d3.json(`./data/tracks/${c.circuitId}.geojson`).catch(() => null)
  );

  Promise.all(trackPromises).then((trackDataArray) => {
    const circuitGeoData = new Map(
      trackDataArray.map((geoData, i) => [circuits[i].circuitId, geoData])
    );
    ...
    circuitGlyphs.each(function (d) {
      ...
      if (d.circuitId === "austin" && activeCircuitIds.has("americas"))
        isActive = true;
      ...
      if (geoData) {
        ...
        const glyphProj = d3
          .geoMercator()
          .fitSize([glyphSize, glyphSize], geoData);
    ...
    if (currentK > trackThreshold) {
      ...
      circuitGlyphs
        ...
        .attr("transform", (d) => {
          ...
          if (
            selectedCircuitId &&
            !isSameCircuit(d.circuitId, selectedCircuitId)
          ) {
      ...
      if (selectedCircuitId) {
        circuitGlyphs
          .filter((d) => isSameCircuit(d.circuitId, selectedCircuitId))
          .raise();
      }
    } else {
      circuitGlyphs
...
function updateInfoPanel(d) {
  ...
  document.getElementById(
    "info-country"
  ).innerText = `${d.Location.locality}, ${d.Location.country}`;
  ...
  if (raceData) {
    ...
    if (raceData.Podium && raceData.Podium.length > 0) {
      raceData.Podium.forEach((res) => {
        ...
        const gridText =
          isNaN(gridPos) || gridPos === 0 ? "Pit" : `P${gridPos}`;

        row.style.borderLeft = `3px solid ${
          teamColors[res.TeamName] || "#555"
        }`;
    ...
    if (activeCircuitIds.has(searchId) || activeCircuitIds.has(d.circuitId)) {
      resultList.innerHTML =
        '<div style="color:#888;text-align:center; padding-top:10px;">Upcoming / No Data</div>';
    } else {
      resultList.innerHTML =
        '<div style="color:#555;text-align:center; padding-top:10px;">Not in 2024 Calendar</div>';
    }
  }
}
...
function renderTeamList(allCircuits) {
  ...
  teamHomeTracksList.forEach((item) => {
    ...
    div.onmouseenter = () => {
      if (circuitGlyphs) {
        const target = circuitGlyphs.filter((d) =>
          isSameCircuit(d.circuitId, circuit.circuitId)
        );
        if (!target.empty()) {
...
d3.select(".resetButton").on("click", () => {
  ...
  document.getElementById("info-country").innerText =
    "Select a track on the globe";
  document.getElementById("enter-track-btn").style.display = "none";
  document.getElementById("podium-list").innerHTML =
    '<div style="color:#666;text-align:center;">Select a red point to see results</div>';
